...
const label = { fontFamily: "'Raleway', sans-serif" as const, fontWeight: 700 as const, fontSize: '0.62rem', letterSpacing: '0.16em', textTransform: 'uppercase' as const, color: 'var(--color-text-muted)', display: 'block', marginBottom: '0.5rem' }
const input = { width: '100%', fontFamily: "'Poppins', sans-serif" as const, fontWeight: 300 as const, fontSize: '0.88rem', color: 'var(--color-text)', border: '1px solid #e0e0e0', borderRadius: '2px', padding: '0.7rem 0.9rem', background: 'white', outline: 'none', boxSizing: 'border-box' as const }
...
function GiftCardsPageInner() { // eslint-disable-line
  ...
  const sectionLabel = { fontFamily: "'Raleway', sans-serif" as const, fontWeight: 700 as const, fontSize: '0.65rem', letterSpacing: '0.18em', textTransform: 'uppercase' as const, color: 'var(--color-text-muted)', marginBottom: '1rem', display: 'block' }
    ...
    return (
      <div style={{ padding: '3rem 2.5rem', maxWidth: '560px' }}>
        <div style={{ textAlign: 'center', padding: '4rem 0' }}>
          ...
          <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.88rem', color: 'var(--color-text-muted)', marginBottom: '1.5rem' }}>
            {formatCurrency(purchaseResult.amount)} gift card
          </p>
          <div style={{ background: 'var(--color-text)', borderRadius: '2px', padding: '1.5rem 2rem', display: 'inline-block', marginBottom: '1.5rem' }}>
            <p style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.6rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--color-text-muted)', marginBottom: '0.5rem' }}>Gift Card Code</p>
            <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '1.6rem', letterSpacing: '0.12em', color: 'var(--color-cta)' }}>{purchaseResult.code}</p>
          </div>
          ...
          <button onClick={() => setPurchaseResult(null)} style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.75rem 1.75rem', borderRadius: '2px', background: 'var(--color-cta)', color: 'white', border: 'none', cursor: 'pointer' }}>
            Send Another
          </button>
        </div>
      </div>
    ...
    <div style={{ padding: '3rem 2.5rem', maxWidth: '620px' }}>
      ...
      <div style={{ display: 'flex', gap: '0', marginBottom: '2.5rem', border: '1px solid #e0e0e0', borderRadius: '2px', overflow: 'hidden', width: 'fit-content' }}>
        {(['buy', 'redeem'] as Mode[]).map(m => (
          <button key={m} onClick={() => setMode(m)} style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.6rem 1.5rem', border: 'none', background: mode === m ? 'var(--color-text)' : 'white', color: mode === m ? 'white' : 'var(--color-text-muted)', cursor: 'pointer' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <button
              ...
              onClick={applyCode}
              style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.7rem 1.5rem', borderRadius: '2px', background: redeemCode.trim() ? 'var(--color-cta)' : '#e0e0e0', color: redeemCode.trim() ? 'white' : '#aaa', border: 'none', cursor: redeemCode.trim() ? 'pointer' : 'not-allowed', whiteSpace: 'nowrap' as const }}
            >
              {redeemLoading ? '...' : 'Apply'}
            </button>
          </div>
          ...
          {redeemResult?.status === 'valid' && (
            <div style={{ background: '#f5ece6', border: '1px solid var(--color-cta)', borderRadius: '2px', padding: '1.25rem 1.5rem', marginBottom: '1rem' }}>
              <p style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--color-cta)', marginBottom: '0.3rem' }}>Valid Gift Card</p>
              <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 100, fontSize: '1.8rem', color: 'var(--color-text)', lineHeight: 1, marginBottom: '0.3rem' }}>{formatCurrency(redeemResult.balance)}</p>
              <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>Balance available · will be applied at checkout</p>
            </div>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem', marginBottom: '0.75rem' }}>
              {AMOUNTS.map(a => (
                <button key={a} onClick={() => { setAmount(a); setCustomAmount('') }} style={{ fontFamily: "'Poppins', sans-serif", fontWeight: customAmount ? 300 : (amount === a ? 500 : 300), fontSize: '0.95rem', padding: '0.7rem', borderRadius: '2px', border: `1px solid ${!customAmount && amount === a ? 'var(--color-cta)' : '#e0e0e0'}`, background: !customAmount && amount === a ? '#f5ece6' : 'white', color: !customAmount && amount === a ? 'var(--color-cta)' : 'var(--color-text)', cursor: 'pointer' }}>
                  {formatCurrency(a)}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem' }}>
              {[{ v: false, label: 'Digital' }, { v: true, label: 'Physical Card' }].map(opt => (
                <button key={String(opt.v)} onClick={() => setIsPhysical(opt.v)} style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.55rem 1.25rem', borderRadius: '2px', border: `1px solid ${isPhysical === opt.v ? 'var(--color-text)' : '#e0e0e0'}`, background: isPhysical === opt.v ? 'var(--color-text)' : 'white', color: isPhysical === opt.v ? 'white' : 'var(--color-text-muted)', cursor: 'pointer' }}>
                  {opt.label}
                </button>
              ))}
            </div>
            {isPhysical && (
              <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.8rem', color: 'var(--color-text-muted)', background: 'var(--color-bg)', padding: '0.75rem 1rem', borderRadius: '2px' }}>
          ...
          {/* Summary + CTA */}
          <div style={{ background: 'var(--color-bg)', border: '1px solid #e8e8e8', borderRadius: '2px', padding: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
              <span style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>Gift Card Value</span>
              <span style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 500, fontSize: '0.95rem', color: 'var(--color-text)' }}>{isNaN(finalAmount) || finalAmount <= 0 ? '—' : formatCurrency(finalAmount)}</span>
            </div>
            <button
              onClick={handlePurchase}
              disabled={!finalAmount || finalAmount <= 0 || checkoutLoading}
              style={{ width: '100%', fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.75rem', letterSpacing: '0.14em', textTransform: 'uppercase', padding: '0.9rem', borderRadius: '2px', background: finalAmount > 0 ? 'var(--color-cta)' : '#e0e0e0', color: finalAmount > 0 ? 'white' : '#aaa', border: 'none', cursor: finalAmount > 0 ? 'pointer' : 'not-allowed' }}
            >
