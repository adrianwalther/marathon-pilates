...
const label = { fontFamily: "'Raleway', sans-serif" as const, fontWeight: 700 as const, fontSize: '0.62rem', letterSpacing: '0.16em', textTransform: 'uppercase' as const, color: '#8a8d83', display: 'block', marginBottom: '0.5rem' }
...
function GiftCardsPageInner() { // eslint-disable-line
  ...
  const sectionLabel = { fontFamily: "'Raleway', sans-serif" as const, fontWeight: 700 as const, fontSize: '0.65rem', letterSpacing: '0.18em', textTransform: 'uppercase' as const, color: '#8a8d83', marginBottom: '1rem', display: 'block' }
    ...
    return (
      <div style={{ padding: '3rem 2.5rem', maxWidth: '560px' }}>
        <div style={{ textAlign: 'center', padding: '4rem 0' }}>
          ...
          <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.88rem', color: '#8a8d83', marginBottom: '1.5rem' }}>
          ...
          <div style={{ background: '#1a1a1a', borderRadius: '2px', padding: '1.5rem 2rem', display: 'inline-block', marginBottom: '1.5rem' }}>
            <p style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.6rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#8a8d83', marginBottom: '0.5rem' }}>Gift Card Code</p>
            <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '1.6rem', letterSpacing: '0.12em', color: '#A76E58' }}>{purchaseResult.code}</p>
          </div>
          ...
          <button onClick={() => setPurchaseResult(null)} style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.75rem 1.75rem', borderRadius: '2px', background: '#A76E58', color: 'white', border: 'none', cursor: 'pointer' }}>
            Send Another
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ padding: '3rem 2.5rem', maxWidth: '620px' }}>
      ...
      <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.85rem', color: '#8a8d83', marginBottom: '2.5rem' }}>Give the gift of movement and restoration.</p>
      ...
      <div style={{ display: 'flex', gap: '0', marginBottom: '2.5rem', border: '1px solid #e0e0e0', borderRadius: '2px', overflow: 'hidden', width: 'fit-content' }}>
        {(['buy', 'redeem'] as Mode[]).map(m => (
          <button key={m} onClick={() => setMode(m)} style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.6rem 1.5rem', border: 'none', background: mode === m ? '#1a1a1a' : 'white', color: mode === m ? 'white' : '#8a8d83', cursor: 'pointer' }}>
      ...
      {mode === 'redeem' ? (
        <div>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <button
              ...
              onClick={applyCode}
              style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.7rem 1.5rem', borderRadius: '2px', background: redeemCode.trim() ? '#A76E58' : '#e0e0e0', color: redeemCode.trim() ? 'white' : '#aaa', border: 'none', cursor: redeemCode.trim() ? 'pointer' : 'not-allowed', whiteSpace: 'nowrap' as const }}
            >
              {redeemLoading ? '...' : 'Apply'}
            </button>
          </div>
          ...
          {redeemResult?.status === 'valid' && (
            <div style={{ background: '#f5ece6', border: '1px solid #A76E58', borderRadius: '2px', padding: '1.25rem 1.5rem', marginBottom: '1rem' }}>
              <p style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#A76E58', marginBottom: '0.3rem' }}>Valid Gift Card</p>
              ...
              <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.78rem', color: '#8a8d83' }}>Balance available · will be applied at checkout</p>
            </div>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.25rem' }}>
              {[{ v: false, label: 'Digital' }, { v: true, label: 'Physical Card' }].map(opt => (
                <button key={String(opt.v)} onClick={() => setIsPhysical(opt.v)} style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.55rem 1.25rem', borderRadius: '2px', border: `1px solid ${isPhysical === opt.v ? '#1a1a1a' : '#e0e0e0'}`, background: isPhysical === opt.v ? '#1a1a1a' : 'white', color: isPhysical === opt.v ? 'white' : '#8a8d83', cursor: 'pointer' }}>
                  {opt.label}
                </button>
              ))}
            </div>
            {isPhysical && (
              <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.8rem', color: '#8a8d83', background: '#FAF7F2', padding: '0.75rem 1rem', borderRadius: '2px' }}>
          ...
          {/* Summary + CTA */}
          <div style={{ background: '#FAF7F2', border: '1px solid #e8e8e8', borderRadius: '2px', padding: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
              <span style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.85rem', color: '#8a8d83' }}>Gift Card Value</span>
            ...
            <button
              ...
              style={{ width: '100%', fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.75rem', letterSpacing: '0.14em', textTransform: 'uppercase', padding: '0.9rem', borderRadius: '2px', background: finalAmount > 0 ? '#A76E58' : '#e0e0e0', color: finalAmount > 0 ? 'white' : '#aaa', border: 'none', cursor: finalAmount > 0 ? 'pointer' : 'not-allowed' }}
            >
