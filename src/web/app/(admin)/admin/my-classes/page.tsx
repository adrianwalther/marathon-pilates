                ...
                >
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.25rem', flexWrap: 'wrap' }}>
                      <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 500, fontSize: '0.9rem', color: 'var(--color-text)' }}>{cls.name}</p>
                      {hasRedFlag && upcoming && (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    <div style={{ textAlign: 'right' }}>
                      <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 400, fontSize: '0.88rem', color: 'var(--color-text)' }}>{confirmedCount}/{cls.max_capacity}</p>
                ...
                {isOpen && (
                  <div style={{ borderTop: '1px solid #f0f0f0', padding: '1.25rem 1.5rem' }}>
                    {cls.bookings.length === 0 ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        <p style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.62rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--color-text-muted)', marginBottom: '0.25rem' }}>
                        ...
                        {cls.bookings.map(b => {
                          ...
                          return (
                            <div key={b.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', background: 'var(--color-bg)', borderRadius: '2px', gap: '1rem' }}>
                              <div style={{ flex: 1 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                                  ...
                                  {isNew && (
                                    <span style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.52rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '0.15rem 0.4rem', borderRadius: '2px', background: '#f5ece6', color: 'var(--color-cta)' }}>New</span>
                                  )}
                              ...
                              {past && (
                                <div style={{ display: 'flex', gap: '0.4rem' }}>
                                  <button
                                    ...
                                    style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.6rem', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.35rem 0.7rem', borderRadius: '2px', border: 'none', background: b.attended === true ? 'var(--color-cta)' : '#f5ece6', color: b.attended === true ? 'white' : 'var(--color-cta)', cursor: 'pointer' }}
                                  >
                                    ✓ Present
                                  </button>
                                  <button
                                    ...
                                    disabled={markingId === b.id}
                                    style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.6rem', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.35rem 0.7rem', borderRadius: '2px', border: 'none', background: b.attended === false ? '#e05555' : '#f5f5f5', color: b.attended === false ? 'white' : 'var(--color-text-muted)', cursor: 'pointer' }}
                                  >
                                    No-show
                                  </button>
                                </div>
