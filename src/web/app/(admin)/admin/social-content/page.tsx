...
export default function SocialContentPage() {
  ...
  const labelStyle = { fontFamily: "'Raleway', sans-serif", fontWeight: 600 as const, fontSize: '0.62rem', letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: 'var(--color-text-muted)', display: 'block' as const, marginBottom: '0.35rem' }

  const STATUS_STYLES: Record<string, { bg: string; color: string }> = {
    pending:  { bg: '#fff8e6', color: '#c8860a' },
    approved: { bg: '#f5ece6', color: 'var(--color-cta)' },
      ...
      <div key={entry.id} style={{ background: 'white', border: '1px solid #eee', borderRadius: '2px', padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
        <div style={{ flex: 1 }}>
          {isAdmin && entry.profiles && (
            <p style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.62rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-cta)', marginBottom: '0.2rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
          ...
          {isAdmin && entry.status === 'pending' && (
            <div style={{ display: 'flex', gap: '0.4rem' }}>
              <button onClick={() => updateStatus(entry.id, 'approved')} style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.58rem', letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0.3rem 0.7rem', background: '#f5ece6', color: 'var(--color-cta)', border: 'none', borderRadius: '2px', cursor: 'pointer' }}>
                Approve
              </button>
      ...
      {showForm && !isAdmin && (
        <form onSubmit={submitEntry} style={{ background: 'white', border: '1px solid #eee', borderRadius: '2px', padding: '2rem', marginBottom: '2rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
            ...
            <div>
              ...
              {formHours && !isNaN(parseFloat(formHours)) && (
                <p style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 300, fontSize: '0.7rem', color: 'var(--color-cta)', marginTop: '0.3rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            ...
            <button type="button" onClick={() => setShowForm(false)} style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '0.7rem 1.2rem', background: 'white', color: 'var(--color-text-muted)', border: '1px solid #e0e0e0', borderRadius: '2px', cursor: 'pointer' }}>
              Cancel
            </button>
          </div>
        </form>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {otherEntries.length > 0 && (
            <div>
              {isAdmin && <p style={{ fontFamily: "'Raleway', sans-serif", fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--color-text-muted)', marginBottom: '0.75rem' }}>History</p>}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
