...
export async function register() {
  // Wrapped so a check error can NEVER crash server startup.
  try {
    ...
    console.error(
      ...
      'After deploy, GET /api/health to verify every integration is wired.\n' +
      '========================================================\n'
    )
  } catch (err) {
    console.error('[env] startup env check failed (non-fatal):', err)
  }
}
