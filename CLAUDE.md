...
**Tests:** pure logic is unit-tested with Vitest (`cd src/web && npm test`) — `lib/nudges` (ranker), `lib/validation` (isUuid), `lib/healthFlags` (the safety guardrail), `lib/winback` (lapsed-detection), `lib/credits` (credit-type), `lib/emails/templates` (transactional copy/wording branches). 55 tests. **CI** (`.github/workflows/ci.yml`) runs `tsc --noEmit` + the suite on every push/PR to `main`. When extracting logic from a route, prefer a pure `lib/*.ts` helper so it's testable.
